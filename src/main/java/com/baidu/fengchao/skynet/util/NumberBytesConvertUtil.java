package com.baidu.fengchao.skynet.util;
...
public class NumberBytesConvertUtil {
    ...
    public static byte[] int2ByteArr(int a) {
        ...
        for (int i = 0; i < SIZE_OF_INT; i++) {
            result[i] = (byte) ((a & INT_BASE[i]) >> ((SIZE_OF_INT - 1 - i) * BITS_PER_BYTE));
        }
        return result;
    }
    ...
    public static int byteArr2Int(byte[] bytes) {
        if (bytes == null || bytes.length == 0 || bytes.length > SIZE_OF_INT) {
            throw new IllegalArgumentException("Required at least one byte, but receive null or empty.");
        }
        int result = 0;
        for (int i = 0; i < bytes.length; i++) {
            result |= bytes[i] << ((bytes.length - 1 - i) * BITS_PER_BYTE)
                    & INT_BASE[INT_BASE.length - bytes.length + i];
        }
        return result;
    }
    ...
    public static byte[] long2ByteArr(long a) {
        ...
        for (int i = 0; i < SIZE_OF_LONG; i++) {
            result[i] = (byte) ((a & LONG_BASE[i]) >> ((SIZE_OF_LONG - 1 - i) * BITS_PER_BYTE));
        }
        return result;
    }
    ...
    public static long byteArr2Long(byte[] bytes) {
        ...
        long result = 0;
        for (int i = 0; i < bytes.length; i++) {
            result |= (long) bytes[i] << ((bytes.length - 1 - i) * BITS_PER_BYTE)
                    & LONG_BASE[LONG_BASE.length - bytes.length + i];
        }
        return result;
    }
}
